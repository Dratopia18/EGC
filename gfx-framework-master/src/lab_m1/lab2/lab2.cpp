...
void Lab2::Init()
{
...

    {
		...
		CreateMesh("tetrahedron", vertices1, indices1);
    }
    ...
    // create a circle
    {
		...
		int n = 100; // numarul de varfuri
		float radius = 1; // raza cercului
		float angle = 0; // unghiul de la care incepe sa deseneze cercul
		float angleStep = 2 * M_PI / n; // pasul cu care se incrementeaza unghiul
		glm::vec3 center = glm::vec3(0, 0, 0); // centrul cercului
		vertices2.push_back(VertexFormat(center, glm::vec3(1, 0, 0), glm::vec3(1, 0, 0))); // adaugam centrul cercului
		// pentru fiecare varf calculam coordonatele si le adaugam in vectorul de varfuri
        for (int i = 0; i < n; i++) {
            vertices2.push_back(VertexFormat(glm::vec3(center.x + radius * cos(angle), 
                center.y + radius * sin(angle), center.z), 
				glm::vec3(1, 0, 0), glm::vec3(1, 0, 0))); // adaugam varful
			indices2.push_back(0); // adaugam centrul cercului
			indices2.push_back(i + 1); // adaugam indicele varfului
			indices2.push_back((i + 1) % n + 1); // adaugam indicele urmatorului varf
			angle += angleStep; // incrementam unghiul
        }
		meshes["circle"] = new Mesh("generated circle"); // creeam mesh-ul
		meshes["circle"]->InitFromData(vertices2, indices2); // initializam mesh-ul
		CreateMesh("circle", vertices2, indices2); // mesh-ul este creat
    }
...
void Lab2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(0, 0.5f, 0), glm::vec3(0.25f));

    // TODO(student): Draw the tetrahedron
	RenderMesh(meshes["tetrahedron"], shaders["VertexNormal"], glm::vec3(1.5f, 0.5f, 0), glm::vec3(0.25f));

    // TODO(student): Draw the square
	RenderMesh(meshes["circle"], shaders["VertexNormal"], glm::vec3(0, 0.5f, 1.5f), glm::vec3(0.25f));
