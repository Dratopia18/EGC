...
Lab1::Lab1()
	: isJumping(false), jumpSpeed(5.0f), jumpHeight(0.0f), gravity(-9.8f)
{
...
int meshIndex = 0;
string meshNames[] = { "box", "bunny", "teapot" };
...
void Lab1::Update(float deltaTimeSeconds)
{
    ...
    // Render the object
    RenderMesh(meshes[meshName], glm::vec3(1, 0.5f, 0.5f), glm::vec3(0.5f));

    // Render the object again but with different properties
    RenderMesh(meshes[meshName], glm::vec3(-1, 0.5f, 1.0f), glm::vec3(0.5f));
	...
	// render the bunny
	RenderMesh(meshes["bunny"], position, glm::vec3(0.01f));

    if (isJumping) {
        jumpHeight += jumpSpeed * deltaTimeSeconds;
        ...
        jumpHeight += gravity * deltaTimeSeconds;
        position.y = jumpHeight;
